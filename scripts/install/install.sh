...
if [[ -n "$WEATHER_API_KEY" && "$WEATHER_API_KEY" != "Skipped" ]]; then
    ENV_TARGET_DIR="$TARGET_CONFIG_DIR/quickshell/widgets/calendar"
...
if [ -f "$REPO_DIR/assets/bin/cava" ]; then
    cp "$REPO_DIR/assets/bin/cava" "$HOME/.local/bin/cava"
...
REPO_FONTS_DIR="$REPO_DIR/assets/fonts"
mkdir -p "$TARGET_FONTS_DIR"

# Copy any remaining local fonts from repo
if [ -d "$REPO_FONTS_DIR" ] && [ -n "$(ls -A "$REPO_FONTS_DIR" 2>/dev/null)" ]; then
...
ZSH_RC="$HOME/.zshrc"
WP_QML="$TARGET_CONFIG_DIR/quickshell/widgets/wallpaper/WallpaperPicker.qml"
WP_DIR="$TARGET_CONFIG_DIR/quickshell/widgets/wallpaper"

# -> Desktop/Laptop Battery Adaptability <-
QS_BAT_DIR="$TARGET_CONFIG_DIR/quickshell/widgets/battery"
