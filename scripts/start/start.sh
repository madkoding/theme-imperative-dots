...
TARGET_QUICKSHELL="${XDG_CONF_HOME}/quickshell"
...
prepare_user_runtime() {
    ...
    rm -rf "${TARGET_QUICKSHELL}"
    cp -a "${THEME_QUICKSHELL}" "${TARGET_QUICKSHELL}"
    find "${TARGET_QUICKSHELL}" -type f -name "*.sh" -exec chmod +x {} +
...
launch_quickshell() {
    local main_qml="${TARGET_QUICKSHELL}/Main.qml"
    local bar_qml="${TARGET_QUICKSHELL}/TopBar.qml"
    ...
    if [[ -f "${TARGET_QUICKSHELL}/widgets/focustime/focus_daemon.py" ]]; then
        pkill -f "focus_daemon.py" >/dev/null 2>&1 || true
        python3 "${TARGET_QUICKSHELL}/widgets/focustime/focus_daemon.py" >/dev/null 2>&1 &
    fi
