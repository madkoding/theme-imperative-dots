...
Item {
    ...
    function updateInfoNodes() {
        ...
        if (window.audioConnected) {
            nodes.push({
                id: "aud-name",
                name: window.audioConnected.name || "Audio Output",
                icon: window.audioConnected.icon || "󰓃",
                action: "Audio Device",
                isInfoNode: true
            });
            nodes.push({
                id: "aud-vol",
                name: window.audioConnected.volume || "0%",
                icon: window.audioConnected.muted ? "󰝟" : "󰕾",
                action: window.audioConnected.muted ? "Muted" : "Volume",
                isInfoNode: true
            });
        }
    ...
    function processAudioJson(textData) {
        ...
        try {
            let data = JSON.parse(textData);
            window.audioPower = data.power || "off";
            window.audioConnected = data.connected || null;
            updateInfoNodes();
        } catch(e) {}
    }
    ...
    Process {
        id: audioPoller
        command: ["bash", window.scriptsDir + "/audio_panel_logic.sh"]
        running: true
        stdout: StdioCollector {
            onStreamFinished: processAudioJson(this.text.trim())
        }
    }
