...
function getLayout(name, mx, my, mw, mh, scale) {
    ...
    let base = {
        ...
        "calendar":  { w: calendarW, h: calendarH, rx: Math.floor((mw / 2) - (calendarW / 2)), ry: s(70, ui), comp: "widgets/calendar/CalendarPopup.qml" },
        
        // Left-aligned: pinned 12px from the left edge
        "music":     { w: s(700, ui), h: s(620, ui), rx: s(12, ui), ry: s(70, ui), comp: "widgets/music/MusicPopup.qml" },
        
        // Right-aligned: pinned 20px from the right edge dynamically
        "network":   { w: s(900, ui), h: s(700, ui), rx: mw - s(920, ui), ry: s(70, ui), comp: "widgets/network/NetworkPopupAlt.qml" },
        ...
        "guide":     { w: guideW, h: guideH, rx: Math.floor((mw / 2) - (guideW / 2)), ry: Math.floor((mh / 2) - (guideH / 2)) + s(24, ui), comp: "widgets/guide/GuidePopup.qml" },

        // Full width, centered vertically
        "wallpaper": { w: wallpaperW, h: wallpaperH, rx: Math.floor((mw - wallpaperW) / 2), ry: Math.floor((mh / 2) - (wallpaperH / 2)), comp: "widgets/wallpaper/WallpaperPicker.qml" },

        // SKWD-inspired app launcher (90% monitor width)
        "launcher":  { w: launcherW, h: launcherH, rx: Math.floor((mw - launcherW) / 2), ry: Math.floor((mh / 2) - (launcherH / 2)), comp: "widgets/launcher/LauncherPopup.qml" },

        // Alt-Tab switcher style card
        "switcher":  { w: s(1700, ui), h: s(620, ui), rx: Math.floor((mw - s(1700, ui)) / 2), ry: Math.floor((mh / 2) - (s(620, ui) / 2)), comp: "widgets/switcher/WindowSwitcher.qml" },
